...
void CodegenCVisitor::visit_eigen_newton_solver_block(const ast::EigenNewtonSolverBlock& node) {
    ...
    printer->add_newline();
    printer->add_line("/* Initialize the newton solver */");
    printer->add_line("const int MAX_ITER = 1e3;");
    printer->add_line("const double EPS = 1e-12;");
    print_statement_block(*node.get_variable_block(), false, false);
    print_statement_block(*node.get_initialize_block(), false, false);
    printer->add_newline();
    printer->add_line("/* Vector to store result of function F(X) */");
    printer->add_line("Eigen::Matrix<double, {}, 1> nmodl_eigen_fm;"_format(N));
    printer->add_newline();
    printer->add_line("/* Matrix to store jacobian of F(X) */");
    printer->add_line("Eigen::Matrix<double, {}, {}> nmodl_eigen_jm;"_format(N, N));
    printer->add_newline();

    printer->add_line("int solver_iteration_count = -1;");
    printer->start_block("while (++solver_iteration_count < MAX_ITER)");

    printer->add_newline();
    printer->add_line("/* Calculate F & J from X. */");
    ...
    printer->add_newline();
    printer->add_line("/* Check for convergence: when F is zero, or close enough. */");
    printer->add_line("/* Here we use sqrt(|F|^2) */");
    printer->add_line("double error = F.norm();");
    printer->add_line("if (error < EPS) {");
    printer->add_line("    break;");
    printer->add_line("}");

    printer->add_line("/* Update X in-place */");
    if (N <= 4) {
        ...
        printer->add_line("#if defined(CORENEURON_ENABLE_GPU) && !defined(DISABLE_OPENACC)");
        printer->add_line("nmodl_eigen_xm -= partialPivLu<N>(nmodl_eigen_jm, nmodl_eigen_fm);");
    ...
    printer->end_block(0);
    printer->add_newline();
    printer->add_line("/* Assign newton solver results in matrix X to state vars */");
